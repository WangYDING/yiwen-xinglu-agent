...
| M6-P1：仓库整理、可复现安装和 CI | 未开始 | 等待用户确认公开身份、历史作者、项目名称、代码与内容许可证及发布形态 |
| M6-P2：演示素材与项目首页 | 未开始 | 精简首页、文档索引、架构图、截图/GIF/视频入口尚未制作 |
...
M6-P0 已在基线 `cf23d198e7a1f48d8a31d60eaa811580977b0a8c` 上完成本地离线发布准备审计。当前没有远程仓库；没有模型、数据库、状态、原始结果或密钥进入 Git 历史，但项目许可证、内容授权、公开身份、既有作者处理和可分发包仍未决定或验证。M6-P1 等待用户完成最小决策清单后另行授权；P0 不修改 Git 配置、不改写历史，也不创建、推送或发布任何远程内容。详见 `docs/M6_RELEASE_READINESS_AUDIT.md` 与 `docs/M6_PORTFOLIO_RELEASE_PLAN.md`。
