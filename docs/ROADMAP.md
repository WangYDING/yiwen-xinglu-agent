...
| Product Completion Plan v1：R1～R6 | R1 待实现；R2～R6 未开始 | 先建立确定性长期成长权威状态，再实现导师教学、结构化记忆、考试传承、内容入口和产品验收 |
...
在继续 M6-P3/P4 或任何公开发布前，项目按 [`PRODUCT_COMPLETION_PLAN_V1.md`](PRODUCT_COMPLETION_PLAN_V1.md) 推进。每一步都必须产生可玩或可审计的真实证据，不能用配置枚举、空数据结构或测试数量冒充产品能力。

| 阶段 | 产品实现主线 | 当前状态 | 最小产物与退出条件 |
|---|---|---|---|
| R1 | 长期状态与确定性成长底座 | **下一阶段唯一允许实现** | 唯一 `ApprenticeshipState`；六项能力、三维关系、来源证据、长期事件、存储/回放/恢复/协调；病例完成后确定性成长；不含导师 Agent |
| R2 | 导师教学闭环 | 未开始 | MentorProfile、MentorAgent、MentorAction、课程布置、有限提示和结构化师评；只读并解释 R1 状态，不能直接修改成长 |
