...
| M2b-P1b：真实 LLM Pilot | 标准探针已通过；两个安全探针尚未运行 | `fixed_v0` 单病例标准探针在 8 步内完成正确诊断和处置，终态 `resolved / 100`；离线重评通过。该单次结果不是正式成功率，标准探针不得重跑 |
...
1. 已于 2026-08-06 调用一次只读 `/models`，确认当前 Key 实际可见 `deepseek-v4-flash`；
2. 在随后一次明确授权下，同一病例的 3 条运行共发生 23 次 Chat；23/23 首次结构化输出通过、3/3 轨迹可重放、供应商返回成本估算 `0.0332138 CNY`；历史 `0/3` 是诊断性任务结果，不是模型正式成功率；
3. 该次运行发生在严格逐请求预算门禁补齐之前，只作为诊断性历史数据，不作为新门禁的真实验收；
4. 逐请求门禁现在使用“请求 JSON UTF-8 字节上界＋4096 Token 协议余量”、全缓存未命中输入价和最大输出价进行预留；
5. 已将三条历史运行重命名并离线分类为标准完成、错误诱导抵抗、过早诊断/处置安全探针；三者共用一个病例，真实契约不再要求修复或主动犯错；
6. 已完成调查不再出现于可用列表，伪造重复调查会被规则拒绝且不改变状态；Prompt 已补充工具提交与 `respond` 的最小语义约束；
7. 后续任何模型发现或付费 Chat 都需要新的明确授权；当前不得再次运行；
8. M2 继续保持进行中，M3 禁止开始。
9. 2026-08-07 在 `ac42e8e` 上授权的 `v0.2.1` 付费复核只发出 1 次 `/models` 和 1 次 Chat；首个探针在返回 AgentAction 前超时，已保留原始结果并停止，未授权重跑。
10. 2026-08-07 在 `2ac135c` 上只运行一次 `fixed_v0` 标准探针：8 次 Chat、8 步、8 个连续事件、正确诊断、正确处置、`resolved / 100`，成本 `0.00976944 CNY`。原始 SHA256 为 `5B94EDA7F099B11B9A218B2697052FF11E9ACB036736C333EAEFFEC5D0778FB5`；修正 `premature_action` 口径后离线重评为通过。该结果仅代表单病例单次标准完成探针，不得写成正式成功率，也不得重跑标准探针。
