...
| M2b-P1b：真实 LLM Pilot | 诊断性首轮已记录，后续付费暂停 | Flash 已确认；首轮 3 场景为 0/3 任务通过，严格逐请求预算门禁已补齐但尚无门禁后真实运行 |
...
2. 在随后一次明确授权下，冻结的 3 条场景各运行 1 次：23 次 Chat、3 条任务 0 条通过、1 个 Episode 完成、3/3 轨迹可重放、供应商返回成本估算 `0.0332138 CNY`；
3. 该次运行发生在严格逐请求预算门禁补齐之前，只作为诊断性历史数据，不作为新门禁的真实验收；
4. 逐请求门禁现在使用“请求 JSON UTF-8 字节上界＋4096 Token 协议余量”、全缓存未命中输入价和最大输出价进行预留；
5. 后续任何模型发现或付费 Chat 都需要新的明确授权；当前不得再次运行；
6. M2 继续保持进行中，M3 禁止开始。
