...
M5 的正式长期连续性使用确定性游戏记忆：从已提交领域事件投影结构化公开事实，再由显式规则影响后续公开对话或行动。语义向量记忆默认关闭，只允许 shadow mode 记录经过过滤的候选，不能进入 Agent Prompt、改变 Agent 行动或玩家状态。详细产品和架构边界见 `docs/M5_MULTI_CASE_VERTICAL_SLICE_PLAN.md`。
