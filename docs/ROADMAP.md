...
M5-P0～P2 已完成，当前三个病例可以独立游玩；P3 尚未开始。M5 的正式长期连续性将使用确定性游戏记忆：从已提交领域事件投影结构化公开事实，再由显式规则影响后续公开对话或行动。语义向量记忆默认关闭，只允许 shadow mode 记录经过过滤的候选，不能进入 Agent Prompt、改变 Agent 行动或玩家状态。详细产品和架构边界见 `docs/M5_MULTI_CASE_VERTICAL_SLICE_PLAN.md`，病例内容审计见 `docs/M5_CASE_DESIGN.md`。
...
M4 的完成只表示 Fake Embedding 下的离线工程契约与安全边界闭环。真实 V1 DoctorAgent 记忆注入、真实玩家收益、生产性能和并发多进程事务仍未验证；它们不构成 M4 退出阻塞。当前 M5-P0～P2 已完成：普通用户可以通过交互式 CLI 独立运行并跨进程恢复三个病例；M5-P3 的确定性跨 Episode 连续性尚未开始。
...
7. M5-P0 已冻结三病例游戏纵向切片；P1 完成本地交互式多病例 Runner，P2 完成两个新病例，当前停止在 P3 之前。
...
M5 从游戏 AI 产品纵向切片开始：P1 已提供普通用户可操作的多病例 Runner，P2 已新增两个完整病例，当前三个病例可独立游玩；P3 将用确定性事件投影实现跨 Episode 连续性，P4 提供 no-LLM/Fake/DeepSeek V0 与语义 shadow 模式，P5 完成三病例试玩验收。自适应教学、Reflection、多 Agent 和网页界面不属于本轮 M5 最小切片。
