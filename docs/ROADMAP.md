...
| M2b-P1：真实 LLM 验证 | 待完成，下一阶段 | 真实适配、小型 Pilot、真实超时/Token/延迟等运行指标 |
| M3：最小 MCP 工具层 | 未开始 | M2b-P1 完成前禁止开始 |
...
M2b-P1 只能在用户选择真实模型供应商并提供可用 API 条件后开始。完成定义如下：

1. 实现一个真实 `LLMAdapter`，配置和密钥与代码分离；
2. 明确请求超时、供应商错误类型、一次格式修复和安全降级日志；
3. 使用已通过 P0 的 3 条 dev 场景运行小型真实模型 Pilot；
4. 保存原始结构化轨迹、模型精确版本、Prompt 版本和运行配置；
5. 只报告实测的格式通过率、任务结果、超时、Token 和延迟，不填补缺失数据；
6. 验证关键状态非法写入为 0、会话串扰为 0、事件可重放、测试与日志完整；
7. 全部 M0–M2b-P0 测试继续通过。

未满足以上条件时，M2 保持“进行中”，不得开始 M3。

## M3 及以后

M3 只计划把现有应用服务包装为最小 MCP 工具，不复制领域规则。M3 尚未开始，M2b-P1 阶段也不创建 MCP 文件或依赖。
